...
//script pour recuperer l'année en cours
    ...
    //function tri par alphabetique d'un tableau
    ...
    // recuperation marque et inject dans le select option
    $.getJSON('TAXCALC.json', function(data){                 //on recup json vehicles
        var results = [];
        var uniqresult = [];
        ...
        $("#marque").append(tabmarque);   //pour chaque entrée on rempli les options
    });


    //on affiche les model dans un deuxieme menu deroulant

    function selectModel(){
        var vh_make = $("#marque").val();
        var tabcar = "";
        var tabmodel = [];
        $("#model").html("<option>model</option>");

        $.getJSON('TAXCALC.json', function(data) {
            $.each(data,  function (index, x) {         //on parcours le json des TAXCALC
            ...
            $("#model").append(tabcar);        //on affiche les données qui corespondent
        })
    }
                ...
                if(x.Vehicle_Description === vh_desc){           //si le modele du select corespond au modele du json
                    ...
                    $.getJSON('CO2TAX.json', function(data) {       //getjson sur le tableau CO2TAX
                        ...
                        function calcul() {
                            ...
                            $('#total').html(totalprice/10000 + '<p><strong>Euros</strong></p>'
                            );


                                $("#divinfos").fadeIn('slow');
                        }
